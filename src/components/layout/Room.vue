    ...
    >
      ...
      <div>
        <p class="font-bold text-lg leading-5">
          {{ truncateText(getCurrentUserToMessage?.name, 20, '...') }}
        </p>
        <p class="text-sm text-zinc-500">
          @{{ truncateText(getCurrentUserToMessage?.username, 20, '...') }}
        </p>
      </div>
    </div>
...
async function fetchPrivateRoom() {
  ...
  const { data } = await chatService.getPrivateRoomByUser(getCurrentUserToMessage.value.username);
